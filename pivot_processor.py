            ...
            try:
                ...
                if not df_finished.empty:
                    if not mapping_df.empty:
                        df_finished, mapped_keys = apply_mapping_and_merge(df_finished, mapping_df, FIELD_MAPPINGS["赛卓-成品库存"])
                        all_mapped_keys.update(mapped_keys)
                ...
                if not product_in_progress.empty:
                    if not mapping_df.empty:
                        product_in_progress, mapped_keys = apply_mapping_and_merge(product_in_progress, mapping_df, FIELD_MAPPINGS["赛卓-成品在制"])
                        all_mapped_keys.update(mapped_keys)
